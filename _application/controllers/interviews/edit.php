...
if( preg_match( '/^c-([\d]*)$/', $PAGE->vars[0], $matches ) )
{
  ...
  if( !$my_contact->load_data( $matches[1] ) )
    $SESS->redirect_msg( 'failure', 'Error creating Interview, Contact not found.' );
...
elseif( preg_match( '/^([\d]*)$/', $PAGE->vars[0], $matches ) )
{
  ...
  if( !$my_obj->load_data( $matches[1] ) )
    $SESS->redirect_msg( 'failure', 'Error: Interview not found.' );

  $id = $matches[1];
}
else
{
  $SESS->redirect_msg( 'failure', 'Error creating Interview: No contact specified.' );
}
...
if( $form_data['submit_form'] )
{
  ...
  $my_contact->set_data( $form_data, F_HTM );
  $my_contact->write_back( );
//print_r( $my_obj );
//exit;
  $SESS->redirect_msg( 'success', 'Interview saved successfully. <a href="interviews/view/'.$my_obj->get('id').'">View Interview</a>' );
}
...
// unless it is the creator of the source.
...
$PAGE->title      = $view_data['add_edit']." Interview";
...
$PAGE->add_script( 'ckeditor/ckeditor.js' );
...
load_view( 'interviews/edit_interview', $view_data );
