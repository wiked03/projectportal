...
if( $form_data['submit_form'] == 'user_del' && $USER->get('level') >= 5 )
{
  ...
  {
    $sql = "DELETE FROM hours WHERE pk_id=".$form_data['user_id'];
    mysql_query( $sql );
    $SESS->redirect_msg( 'success', 'Hour deleted successfully.' );
  }
  else
    $SESS->redirect_msg( 'failure', 'There was a problem deleting the hour.' );
}
...
$list->add_column( 'int_amount', 'Hours', 125 );
$list->add_column( 'notes', 'Notes', 325 );
...
$sql = "SELECT *, ex.pk_id AS id, p.pk_id AS proj_id, p.name AS proj_name, 
              c.pk_id AS con_id, CONCAT_WS(' ', c.first_name, c.last_name) AS con_name, 
              int_amount as int_amount, ex.notes as notes
        FROM hours AS ex 
		  LEFT JOIN projects AS p ON p.pk_id = ex.fk_project_id
		  LEFT JOIN contacts AS c ON ex.fk_contractor_id = c.pk_id " . $where . "
        ORDER BY int_date, con_name";
...
while( $res = mysql_fetch_assoc( $result ) )
{
  ...
  $list->set_row_link( 'admin/hours/edit/'.$res['id'] );
  $list->disable_row_link( 'actions' );

  $val = '<a href="admin/hours/edit/'.$res['id'].'" title="edit"><img width="16" height="16" src="img/icons/pencil.png"/></a>';
  $val .= '<a href="javascript:delete_hour('.$res['id'].')" title="delete"><img width="16" height="16" src="img/icons/trash.png"/></a>';

  $list->set_row_data( 'actions', $val ); 

}

$sql_total = "SELECT sum(int_amount) as total
        FROM hours AS ex 
		  LEFT JOIN projects AS p ON p.pk_id = ex.fk_project_id
		  LEFT JOIN contacts AS c ON ex.fk_contractor_id = c.pk_id " . $where;
$result_total = mysql_query( $sql_total );

while( $res_total = mysql_fetch_assoc( $result_total ) ){
  $total_hours = $res_total['total'];
}

if( $search_string['all'] )
  $search_str = 'Showing all hours.';
elseif( $search_string )
  $search_str = 'Showing hours where: '.implode(' and ', $search_string ).'.';

//-------------------------------------------------------------------------------------------
// Page
//-------------------------------------------------------------------------------------------

$PAGE->title      = "Hours Report";
...
<div class="page_content">
 <div class="page_content_full">
  ...
  <h1>
    ...
    Hours Report - <? echo $project_name; ?>
  </h1>
<? 
//------------------------------------------------------------ 
